...
loop() { # src dst width maxSeconds
  ffmpeg -y -v error -i "$1" -t "$4" -an -vf "scale=$3:-2:flags=lanczos,fps=30" \
    -c:v libx264 -preset slow -crf 30 -profile:v high -pix_fmt yuv420p -movflags +faststart "$2"
}
full() { # src dst width
  ffmpeg -y -v error -i "$1" -vf "scale=$3:-2:flags=lanczos,fps=30" \
...
poster() { # src dst width
  ffmpeg -y -v error -ss 0.5 -i "$1" -frames:v 1 -vf "scale=$2:-2" -q:v 5 "$3"
}
...
full "$F/organic-ugc-holafly.mp4" "$OUT/reviews/organic.mp4" 720
for n in product service organic; do poster "$OUT/reviews/$n.mp4" 720 "$OUT/posters/review-$n.jpg"; done
